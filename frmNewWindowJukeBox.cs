...
namespace RaagaHacker
{
	...
	public class frmNewWindowJukeBox : System.Windows.Forms.Form
	{
        ...
        private ulong m_lCumulativeBytesCount = 0;
...
//                                framePlayer.execScript(sJSTitleChange, "JavaScript");
		...
		private void DataArrived(IntPtr data, int size)
		{
			try
			{
				...
				if (m_Writer != null)
				{
                    ...
                    m_lCumulativeBytesCount += m_Writer.BytesWritten;
				}

                //If invoked syncronously, the thread-block will jeoperdize the waveIn 
                //recording. So following invocation should be prohibited...
                //
                //UpdateAudioLevelUI(m_RecBuffer, size);
                //
                //Either use explicit Thread.Start() stuff or Delegate.Invoke(ThreadPool) etc.
                object[] values = new object[]{m_RecBuffer,size};
                System.Threading.ThreadPool.QueueUserWorkItem(
                    new System.Threading.WaitCallback(UpdateAudioLevelUI),values);
			...
			catch(Exception _e)
			{
				...
				{
					frmException frm = new frmException();
                    frm.ExceptionDialogTitle = "DataArrived: Recording problem ";
					frm.ErrorMessage = _e.Message;
					frm.StrackTrace = _e.StackTrace;
					if (frm.ShowDialog() == DialogResult.OK)
					{
        ...
        private void UpdateAudioLevelUI(object state)
        {
            ...
            long    i = 0;
            long    iVal4 = bufferSize/4, iVal2 = bufferSize/2;
            long    iLeftAudioValue = 0, iRightAudioValue = 0;
            long    iLeftAudioValueSquared = 0, iRightAudioValueSquared = 0;
            double  dblRMSLeft = 0.0f,  dblRMSRight = 0.0f;
            double  dbldbLeft = 0.0f, dbldbRight = 0.0f;
            try
            {
                while (i < (size - 2))
                {
                    ...
                    //We are calculating RMS value. buffrSise/2 is the sample mean
                    iLeftAudioValue = data[i] - iVal4;
                    iLeftAudioValueSquared += iLeftAudioValue * iLeftAudioValue;

                    //Get the right channel data for same sample
                    i++;

                    //We are calculating RMS value. buffrSise/2 is the sample mean
                    iRightAudioValue = data[i] - iVal4;
                    iRightAudioValueSquared += iRightAudioValue * iRightAudioValue;
                ...
                //RMS values
                dblRMSLeft = ((double)(iLeftAudioValueSquared / iVal2) * 
                              (double)(iLeftAudioValueSquared / iVal2));
                dblRMSRight = ((double)(iRightAudioValueSquared / iVal2) *
                                (double)(iRightAudioValueSquared / iVal2));

                //Get the values in db
                dbldbLeft = 20 * (Math.Log(dblRMSLeft) / Math.Log(10.0f));
                dbldbRight = 20 * (Math.Log(dblRMSRight) / Math.Log(10.0f));

                if (m_wndParent != null)
                {
                    m_wndParent.SetChannelValue((int)dbldbLeft, (int)dbldbRight);
                }
            ...
            catch (Exception _e)
            {
                ...
                {
                    frmException frm = new frmException();
                    frm.ExceptionDialogTitle = "UpdateAudioLevelUI: Recording problem ";
                    frm.ErrorMessage = _e.Message;
                    frm.StrackTrace = _e.StackTrace;
                    if (frm.ShowDialog() == DialogResult.OK)
                    {
