...
namespace MyClasses
{
	...
	public class Packet32h
	{
		...
		private WinService.SERVICE_STATUS SStat;
		...
		private Function.OSVERSIONINFO mOSInfo;
		...
		public Packet32h()
		{
			Function.GetOSInfo( ref mOSInfo , ref mOSName );
			...
			mNPFDriverPath = "c:\\windows\\system32\\drivers\\npf.sys";
		...
		public bool PacketStartDriver()
		{
			int SchService = 0;
			int Result = 0;
			int QuerySStat = -1;

			if( mScmHandle == 0 ) OpenServiceControlManager();
			...
			if( mScmHandle > 0 )
			{
				...
				SchService = WinService.OpenService( mScmHandle,
					mNPFServiceNamePtr,
				...
				QuerySStat = WinService.QueryServiceStatus( SchService, ref SStat );
				if( QuerySStat == 0 || SStat.dwCurrentState != WinService.SERVICE_RUNNING)
				{
					Result = WinService.StartService( SchService , 0, 0 );
					if( Result == 0 )
					{
						mPacketErrorCode = Function.GetLastError();
						if( mPacketErrorCode > 0 )
		...
		public bool PacketRemoveDriver()
		{
			int SchService = 0;
			int Result = -1;
			...
			if( mScmHandle > 0 )
			{
				...
				if( SchService > 0 )
				{
					Result = WinService.DeleteService( SchService );
					mPacketErrorCode = Function.GetLastError();
					if( mPacketErrorCode > 0 )
						mPacketErrorMessage = Function.GetSystemErrorMessage( mPacketErrorCode );
		...
		public bool IsNPFInRegistry()
		{
			RegistryKey RKey;

			RKey = Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Services\\NPF" , false );
		...
		public bool CreateNPFEntry()
		{
			RegistryKey RKey;

			RKey = Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Services\\NPF" , true );
			if( RKey == null ) return false;

			RKey.SetValue("Service" , "c:\\winnt\\system32\\drivers\\npf.sys" );
			RKey.Close();
		...
		public bool DeleteNPFEntry()
		{
			RegistryKey RKey;

			RKey = Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Services" , true );
			if( RKey == null ) return false;

			RKey.DeleteSubKey( "NPF" , false );
			RKey.Close();
		...
		{
			if( ( mOSName == "WIN2000" ) || ( mOSName == "WINXP" ) )
				return PacketGetAdapterNamesWin2000();
			else if ( mOSName == "WINNT" )
				return PacketGetAdapterNamesWinNT();
			else if ( ( mOSName == "WIN95" ) )
				return PacketGetAdapterNamesWin95();
			else if ( ( mOSName == "WIN98" ) || ( mOSName == "WIN98ME" ) )
				return PacketGetAdapterNamesWin98();

			return false;

		}

		public bool PacketGetAdapterNamesWin2000()
		{
		...
		public bool PacketGetAdapterNamesWinNT()
		{
			RegistryKey AdapKey;
			RegistryKey OneAdapKey;
			string [] SubKeys;
			...
			try
			{
				AdapKey = Registry.LocalMachine.OpenSubKey("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\NetworkCards", false );

				if( AdapKey == null )
				{
					mPacketErrorMessage = "PacketGetAdapterNames: Registry Query ( Network card not found )";
					return false;
				}
				...
				for( i = 0; i < SubKeys.GetLength(0); i ++ )
				{
					...
					DevName = "NPF_" + AllAdapterNames[i];
				...
				int cnt = 0;
				for( i = 0; i < AllAdapterNames.Length; i ++ )
				{
					if( AllAdapterNames[i] != "" ) cnt ++;
				}
		...
		public bool PacketGetAdapterNamesWin95()
		{
			RegistryKey AdapKey;
			string [] AllAdapterNames;
			string [] AllDescriptions;
			...
			try
			{
				AdapKey = Registry.LocalMachine.OpenSubKey("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\NetworkCards\\1", false );
				...
				AllAdapterNames = new string[ 1 ];
				AllDescriptions = new string[ 1 ];

				AllAdapterNames[0] = ( string ) AdapKey.GetValue("ServiceName");
				AllDescriptions[0] = ( string ) AdapKey.GetValue("Title");
				DevName = "\\Device\\NPF_" + AllAdapterNames[0];
		...
		public bool PacketGetAdapterNamesWin98()
		{
			RegistryKey AdapKey;
			string [] AllAdapterNames;
			string [] AllDescriptions;
		...
		{
			if( ( mOSName == "WIN2000" ) || ( mOSName == "WINXP" ) )
				return PacketOpenAdapterWin2000( pAdapterName );
			else if ( mOSName == "WINNT" )
				return PacketOpenAdapterWinNT( pAdapterName );
			else if ( ( mOSName == "WIN95" ) )
				return PacketOpenAdapterWin95( pAdapterName );
			else if ( ( mOSName == "WIN98" ) || ( mOSName == "WIN98ME" ) )
				return PacketOpenAdapterWin98( pAdapterName );
		...
		public bool PacketOpenAdapterWin2000( string pAdapterName )
		{
			int Result = 0;
			int error = 0;
		...
		public bool PacketOpenAdapterWinNT( string pAdapterName )
		{
			int Result = 0;
			int error = 0;
			char [] SymbolicLink = new char[128];
			...
			string Symb = "";
			if( mOSName == "WIN2000" )
				Symb = "\\\\.\\" + AdapterName.Substring( 8 , AdapterName.Length - 8 );
			else if( mOSName == "WINNT" )
				Symb = "\\\\.\\" + AdapterName;
		...
		public bool PacketOpenAdapterWin95( string pAdapterName )
		{
			int Result = 0;
			int error = 0;
			char [] SymbolicLink = new char[128];
		...
		public bool PacketOpenAdapterWin98( string pAdapterName )
		{
			int Result = 0;
			int error = 0;
			char [] SymbolicLink = new char[128];
		...
		public bool OpenServiceControlManager()
		{
			mScmHandle = WinService.OpenSCManager( 0, 0, WinService.SC_MANAGER_ALL_ACCESS );
			if( mScmHandle > 0 ) return true;

			return false;
		}


		public void CloseServiceControlManager()
		{
		...
		{
			int result = 0;
			IntPtr IPtr1 = Marshal.StringToCoTaskMemAnsi( mNPFServiceName );
			IntPtr IPtr2 = Marshal.StringToCoTaskMemAnsi( mNPFServiceDesc );
			IntPtr IPtr3 = Marshal.StringToCoTaskMemAnsi( mNPFDriverPath );
			int mNPFServiceNamePtr = ( int ) IPtr1;
			int mNPFServiceDescPtr = ( int ) IPtr2;
			int mNPFDriverPathPtr = ( int ) IPtr3;

			Function.CopyFileTo( mOSName );
