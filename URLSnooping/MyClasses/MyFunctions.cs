...
namespace MyClasses
{
	...
	public class Function
	{
		...
		public struct TRANSPARENCY_OPTIONS
		{
			public int Value;
		...
		public struct COLUMN_DEFAULT_OPTIONS
		{
			public int No;
			public int Time;
			public int Source;
		...
		//**********************************************************************
		[DllImport("kernel32.dll", EntryPoint="GlobalAlloc")] unsafe public extern static char *
		...
		[DllImport("kernel32.dll")] public static extern int
			SearchPath(
				int lpPath,	// address of search path 
				int lpFileName,	// address of filename 
				int lpExtension,	// address of extension 
				int nBufferLength,	// size, in characters, of buffer 
				int lpBuffer,	// address of buffer for found filename 
				int lpFilePart ); 	// address of pointer to file component 
		//**********************************************************************
		...
		//**********************************************************************
		[DllImport("user32.dll")] public extern static int
			SetWindowLong(
			int hWnd,	// handle of window
		...
		public static void HighlightText( RichTextBox Rtx , TreeNode MNode , ref StatusBar SBar )
		{
			...
			SBar.Panels[0].Text = "Index : " + intArray[ 0 ].ToString() + " , Length : " + intArray[ 1 ].ToString();
...
// </KEITH>
			...
			)
		{
			...
			if( ( IndexOfNode != - 1 ) && ( LengthOfNode != -1 ) )
				Function.SetPosition( ref mNode , IndexOfNode , LengthOfNode , true );
			...
			)
		{
			...
			switch( ReadType )
			{
				case Const.STR_ASCII :
					Tmp = NodeName + " : " + GetString( Data , ref Index , false );
					break;

				case Const.STR_UNICODE :
					Tmp = NodeName + " : " + GetString( Data , ref Index , true );
					break;

				case Const.STR_ASCII_UNICODE :
					Tmp = NodeName + " : " + GetUnicodeOrAscii( Data , ref Index );
					break;

				case Const.STR_NB_NAME_NORMAL :
					Tmp = NodeName + " : " + GetNetBiosNameSerial( Data , ref Index , ref NNum );
					break;

				case Const.STR_NB_NAME_DECODE :
					Tmp = NodeName + " : " + GetNetBiosName( Data , ref Index , ref NNum );
					break;

				case Const.STR_MAC_ADDRESS :
					Tmp = NodeName + " : " + GetMACAddress( Data , ref Index );
					break;

				case Const.STR_IP_ADDRESS :
					Tmp = NodeName + " : " + GetIpAddress( Data , ref Index );
					break;

				case Const.STR_FORMAT_HEX :
					Tmp = NodeName + " : " + GetString( Data , ref Index , "x02" , ReadLength );
					break;
			...
			SetPosition( ref mNode , kk , Index - kk , IsBaseNode );
			...
			)
		{
			...
			{
				Tmp = "[ Malformed Data. There is nothing to get more fielfs from the captured data. Parsed the end of data. The current data index  is " + Index.ToString();
				mNode.Text = Tmp;
				Index = -1;
				return mNode;
			}
		...
		public static void AddNodeItem(
			ref TreeNode mNode,
			string NodeName,
			string NodeValue,
			int IndexOfNode,
			...
			)
		{
			...
			{
				if( NodeValue != "" )
					mNode.Nodes.Add( NodeName + " : " + NodeValue );
				else
					mNode.Nodes.Add( NodeName );
			}
			else
				mNode.Nodes.Add( NodeValue );

			if( ( IndexOfNode != - 1 ) && ( LengthOfNode != -1 ) )
				Function.SetPosition( ref mNode , IndexOfNode , LengthOfNode , false );
			...
			)
		{
			...
			{
				case Const.STR_ASCII :
					Tmp = GetString( Data , ref Index , false );
					break;

				case Const.STR_UNICODE :
					Tmp = GetString( Data , ref Index , true );
					break;

				case Const.STR_ASCII_UNICODE :
					Tmp = GetUnicodeOrAscii( Data , ref Index );
					break;

				case Const.STR_NB_NAME_NORMAL :
					Tmp = GetNetBiosNameSerial( Data , ref Index , ref NNum );
					break;

				case Const.STR_NB_NAME_DECODE :
					Tmp = GetNetBiosName( Data , ref Index , ref NNum );
					break;

				case Const.STR_MAC_ADDRESS :
					Tmp = GetMACAddress( Data , ref Index );
					break;

				case Const.STR_IP_ADDRESS :
					Tmp = GetIpAddress( Data , ref Index );
					break;

				case Const.STR_FORMAT_HEX :
					Tmp = GetString( Data , ref Index , "x02" , ReadLength );
					break;
			...
			Tmp = NodeName + " : " + Tmp;
			mNode.Nodes.Add( Tmp );
			SetPosition( ref mNode , kk , Index - kk , false );

			return mNode.Nodes[ mNode.Nodes.Count - 1 ];
			...
			)
		{
			...
			Tmp = NodeName + " : " + ReFormatString( NodeValue , NodeOptionalValue );
			mNode.Nodes.Add( Tmp );
			SetPosition( ref mNode , Index - 1 , 1 , false );
			...
			)
		{
			...
			Tmp = NodeName + " : " + ReFormatString( NodeValue , NodeOptionalValue );
			mNode.Nodes.Add( Tmp );
			SetPosition( ref mNode , Index - 2 , 2 , false );
			...
			)
		{
			...
			Tmp = NodeName + " : " + ReFormatString( NodeValue , NodeOptionalValue );
			mNode.Nodes.Add( Tmp );
			SetPosition( ref mNode , Index - 4 , 4 , false );
			...
			)
		{
			...
			Tmp = NodeName + " : " + ReFormatString( NodeValue , NodeOptionalValue );
			mNode.Nodes.Add( Tmp );
			SetPosition( ref mNode , Index - 8 , 8 , false );
		...
		public static void AddInfoText( ref ListViewItem LItem , string TextToBeAdded )
		{
			string Tmp = "";

			Tmp = LItem.SubItems[ Const.LIST_VIEW_PACKET_NO_INDEX ].Text;
			Tmp += ", " + TextToBeAdded;
			LItem.SubItems[ Const.LIST_VIEW_PACKET_NO_INDEX ].Text = Tmp;

		}

		public static void GetDateTime( uint Seconds , ref string Date , ref string Time )
		{
			string [] Months = new string[13];
			string [] Days = new string[8];
			uint YearMultiplier = 60 * 60 * 24 * 365;
			uint MonthMultiplier = 60 * 60 * 24 * 365 / 12;
			uint DayMultiplier = 60 * 60 * 24;
			uint HourMultiplier = 60 * 60;
			uint MinuteMultiplier = 60;
			...
			uint Year = Seconds / YearMultiplier;
			Seconds -= Year * YearMultiplier - Year / 4; Year += 1970;
			uint Month = Seconds / MonthMultiplier;
			Seconds -= Month * MonthMultiplier - Month * 4 / 10;
			uint Day = Seconds / DayMultiplier;
			Seconds -= Day * DayMultiplier;
			uint Hour = Seconds / HourMultiplier;
			Seconds -= Hour * HourMultiplier;
			uint Minute = Seconds / MinuteMultiplier;
			Seconds -= Minute * MinuteMultiplier;

			Date = Days[ Day / 7 ] + " " + Months[ Month ] + " " + Day.ToString() + ", " + Year.ToString();
			Time = Hour.ToString() + ":" + Minute.ToString();

		}


		public static void UpdateStatisticsItem( ListView LVw , string ItemName , string CountStr , string PctStr , Color OldClr , Color NewClr )
		{
			int Index = -1;
			int i = 0;
			string OldString = "";
			...
			ChangeColor = false;
			OldString = LVw.Items[ Index ].SubItems[1].Text;
			if( ( CountStr != "" ) && ( OldString != CountStr ) )
			{
				LVw.Items[ Index ].SubItems[1].Text = CountStr;
				ChangeColor = true;
			}

			OldString = LVw.Items[ Index ].SubItems[2].Text;
			if( ( PctStr != "" ) && ( OldString != PctStr ) )
				LVw.Items[ Index ].SubItems[2].Text = PctStr;

			if( ChangeColor )
			{
				if( LVw.Items[ Index ].SubItems[0].ForeColor == OldClr )
					LVw.Items[ Index ].SubItems[0].ForeColor = NewClr;

				if( LVw.Items[ Index ].SubItems[1].ForeColor == OldClr )
					LVw.Items[ Index ].SubItems[1].ForeColor = NewClr;

				if( LVw.Items[ Index ].SubItems[2].ForeColor == OldClr )
					LVw.Items[ Index ].SubItems[2].ForeColor = NewClr;
			}

		}


		public static string FindHexString( string SearchStr , byte [] Data , int StartPos , ref int FoundPos , ref int NextSearchPos )
		{
			string Tmp = "";
			int i = 0;
			...
			if( StartPos >= Data.Length ) 
			{
				...
				return "Starting position exceeds the upper bound of the search area ...";
			}

			int Remaining = SearchStr.Length / 2;
			if( Remaining > 0 )
			{
				NextSearchPos = -1;
				FoundPos = -1;

				return "Search string is not formatted as a hex string ...";
			}

			if( ( StartPos + SearchStr.Length ) >= Data.Length )
			{
				NextSearchPos = -1;
				FoundPos = -1;
				return "";
			...
			{
				Tmp += Data[i].ToString("x02");
			}

			FoundPos = Tmp.IndexOf( SearchStr , 0 ) / 2;
			NextSearchPos = FoundPos + SearchStr.Length / 2;
		...
		public static void MakeTransparent( int Hwnd , string mOSName , int LayerDensity )
		{
			int Result = -1;
			string ResultStr = "";

			byte LayerDensityValue = 0;
			if( mOSName != "WIN2000" ) return;

			if( LayerDensity > 100 ) LayerDensity = 100;
			LayerDensityValue = (byte) ( LayerDensity * 5 / 2 );

			SetLastError( 0 );
			Result = SetWindowLong( Hwnd, GWL_EXSTYLE, GetWindowLong( Hwnd, GWL_EXSTYLE) | WS_EX_LAYERED );
			if( Result == 0 )
				ResultStr = GetSystemErrorMessage( GetLastError() );

			SetLastError( 0 );
			Result = SetLayeredWindowAttributes( Hwnd , 0, LayerDensityValue , LWA_ALPHA );
			if( Result == 0 )
				ResultStr = GetSystemErrorMessage( GetLastError() );

		}

		public static void ArrangeListView( ref ListView LVw , COLUMN_OPTIONS COptions , COLUMN_DEFAULT_OPTIONS CDefOptions )
		{
			LVw.Columns[ 0 ].Width = COptions.No ? CDefOptions.No : 0;
			LVw.Columns[ 1 ].Width = COptions.Time ? CDefOptions.Time : 0;
			LVw.Columns[ 2 ].Width = COptions.Source ? CDefOptions.Source : 0;
			LVw.Columns[ 3 ].Width = COptions.SourcePort ? CDefOptions.SourcePort : 0;
			LVw.Columns[ 4 ].Width = COptions.Destination ? CDefOptions.Destination : 0;
			LVw.Columns[ 5 ].Width = COptions.DestinationPort ? CDefOptions.DestinationPort : 0;
			LVw.Columns[ 6 ].Width = COptions.Protocol ? CDefOptions.Protocol : 0;
			LVw.Columns[ 7 ].Width = COptions.Sequence ? CDefOptions.Sequence : 0;
			LVw.Columns[ 8 ].Width = COptions.Acknowledge ? CDefOptions.Acknowledge : 0;
			LVw.Columns[ 9 ].Width = COptions.Info ? CDefOptions.Info : 0;
